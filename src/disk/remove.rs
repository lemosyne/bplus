...
impl<K, V> BPTree<K, V> {
        ...
        Q: Ord,
    {
        ...
        if let Node::Leaf(node) = cursor.access_mut(&self.path)? {
            ...
            if let Node::Internal(parent) = node.parent.unwrap().access_mut(&self.path)? {
                // Check if the left sibling has any extra keys.
                if cursor_index > 0 {
                    if let Node::Leaf(left_sibling) =
                        parent.children[cursor_index - 1].access_mut(&self.path)?
                    {
        ...
        Q: Ord,
    {
        ...
        if let Node::Internal(node) = cursor.access_mut(&self.path)? {
            ...
            if !node.is_underfull(self.order) || Some(&cursor) == self.root.as_ref() {
                return Ok(());
            }
                ...
                if cursor_index > 0 {
                        ...
                        parent.children[cursor_index - 1].access_mut(&self.path)?
                    {
                        ...
                        if left_sibling.has_extra_keys(self.order) {
                            ...
                            match node.children[0].access_mut(&self.path)? {
                ...
                if cursor_index + 1 < parent.children.len() {
                        ...
                        parent.children[cursor_index + 1].access_mut(&self.path)?
                    {
                        if right_sibling.has_extra_keys(self.order) {
                            ...
                            match node.children[node.children.len() - 1].access_mut(&self.path)? {
